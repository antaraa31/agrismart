 ...
 * Results are cached per (crop, location) for 15 minutes to avoid hammering NewsAPI on every page load.
 */
...
const fetchPestNews = async (crop, location = '') => {
  if (!hasApiKey()) return [];
  const cacheKey = `${String(crop).toLowerCase()}|${String(location).toLowerCase()}`;
  return withCache('news:pest', cacheKey, TTL_MS, async () => {
    try {
...
const fetchAgriNews = async (location = '') => {
  if (!hasApiKey()) return [];
  const cacheKey = String(location).toLowerCase();
  return withCache('news:agri', cacheKey, TTL_MS, async () => {
    try {
      const apiKey = process.env.NEWS_API_KEY;
